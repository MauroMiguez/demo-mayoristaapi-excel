...
# Carpeta donde se guardan los pedidos en Excel
...
def generate_excel(order_id: int, order: Order):
    ...
    for item in order.items:
        ws.append([item.producto, item.color, item.talle, item.cantidad, ""])
    ...
    row += 1  # 👈 mismo nivel que el if, dentro del for
...
def image_url_to_path(image_url: str):
    """
    Convierte /static/images/x.jpg a static/images/x.jpg
    """
    if not image_url:
        return None
