...
async function enviarPedido() {
  try {
    ...
    // Si querés descargar el Excel automáticamente:
    if (data.excel_url) {
...
function enviarPedidoPrueba() {
        ...
        items: [
            {
                ...
                talle: "M",
                cantidad: 1,
                imagen: "/images/test/ejemplo.jpg"  // ruta relativa a tu carpeta static
            }
        ]
    };
    ...
    .then(data => {
        ...
        // Mostrar link para descargar Excel
        const link = document.createElement("a");
        link.href = data.excel_url;
        link.textContent = "Descargar Excel del pedido";
        link.download = `pedido_${data.order_id}.xlsx`;
